...
class hntf2d_control_law:
    ...
    def realws_to_disk_transformation(self):  # calculate the harmonic transformation that maps the real workspace to the sphere world
        ...
        try:  # try to calculate the transformation that maps the real workspace to the unit disk
            if self.harmonic_map_object_is_built and not self.wtd_transformation_is_built:  # check if the real workspace to disk transformation has not been done yet
                ...
                for i in range(len(self.inner_boundaries)):  # loop through all the inner boundaries
                    inner_boundaries.append(self.inner_boundaries[i].tolist())  # make sure the inner boundaries are lists
                # append the boundaries (outer and inner) to the HarmonicMap2D object
                self.harmonic_map_2D.boundary_append(np.array(outer_boundary))  # append the outer boundary to the HarmonicMap2D object
                for k in range(len(inner_boundaries)):  # loop through all the inner boundaries
                    self.harmonic_map_2D.boundary_append(np.array(inner_boundaries[k]))  # append each inner boundary to the HarmonicMap2D object
                ...
                self.unit_circle = np.array([u0, v0]).T  # create a unit circle array with coordinates u0, v0
                ...
                self.harmonic_map_2D.solve(u0, v0)  # compute the harmonic transformation
                # calculate the punctures of the harmonic transformation
                self.q_i_disk = np.array(self.harmonic_map_2D.puncts())  # calculate the punctures of the harmonic transformation
    ...
    def realws_to_unit_disk_mapping(self, p):  # map the point p from the real workspace to the sphere world (unit disk)
        ...
        try:  # try to map the point p from the real workspace to the unit disk
            if self.harmonic_map_object_is_built and self.wtd_transformation_is_built:  # check if the HarmonicMap2D object has been created and the real workspace to disk transformation has been done
                p = np.array(p).reshape(-1, 1)  # make sure p is a numpy array
                q = np.array(self.harmonic_map_2D.map(p)).flatten()  # map the point p to the sphere world (unit disk)
                # print("Point mapped to unit disk successfully!")  # print a message to the console
    ...
    def realws_to_unit_disk_jacobian(self, p):  # calculate the jacobian of the transformation from the real workspace to the sphere world at the given point p
        ...
        try:  # try to calculate the jacobian of the transformation at the point p
            if self.harmonic_map_object_is_built and self.wtd_transformation_is_built:  # check if the HarmonicMap2D object has been created and the real workspace to disk transformation has been done
                p = np.array(p).reshape(-1, 1)  # make sure p is a numpy array
                jacobian = np.array(self.harmonic_map_2D.jacob(p))  # calculate the jacobian matrix of the transformation at the point p
                # print("Jacobian matrix connecting real workspace and unit disk computed successfully!")  # print a message to the console
