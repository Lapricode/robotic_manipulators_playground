...
In the sixth menu (left side of the picture below), users can copy and digitally reconstruct the actual workspace along with any obstacles. Initially, a 2D plane is created to represent the surface on which the obstacles are positioned. The length, width, and configuration of this plane are defined either manually or through automated processes using the camera (a button for this feature is available in the fifth menu). Additionally, users can evaluate the suitability of the plane in terms of the mobility it offers to the robotic arm (regarding to reachable points, kinematic singularities etc.) and can save it for future use if needed.

Next, one of the saved images of potential workspaces is selected, and an attempt is made to detect the boundaries of the visible obstacles in the image (right side of the picture below). For simulation and 3D printing purposes, options are provided to create obstacles meshes and generate corresponding STL files. Overall, there is a close interaction between the functions of the fifth and sixth menus, meaning users may need to switch between them to complete tasks related to these features.

![image](https://github.com/user-attachments/assets/ab8f77bc-2541-4e51-913b-82ae25272798)
